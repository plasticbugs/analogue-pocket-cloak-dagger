...
// board -- Cloak & Dagger's own audio path ends at the op-amp summing stages
// modelled in cloak_audio.sv, and this sits after all of it.
//
// Ported unchanged in its sound from the Punch-Out!! core's po_reverb (by way
// of Pole Position's pp_reverb, which is the same filter made stereo). Cloak &
// Dagger is mono, so this is the original mono arrangement.
//
...
// speaker would show.
//
// One sample per ce -- cloak_audio's 48.08 kHz tick, with `in` valid alongside
// it. The work is sequential and takes nine clocks of the ~832 available.
// Delay lines in block RAM, 3 x 2048 x 16. All arithmetic is shifts and adds;
// every sum is saturated. Mode 0 passes the input through with the same
// one-sample latency as the other modes.
//------------------------------------------------------------------------------
...
module cloak_reverb (
    input  wire               clk,
    input  wire               reset,
    input  wire               ce,          // one 48 kHz tick; `in` valid with it
    input  wire        [1:0]  mode,        // 0 off, 1 light, 2 medium, 3 heavy
    input  wire signed [15:0] in,
    output logic signed [15:0] out         // valid until the next ce, nine clocks after this one
);
    ...
    cloak_delayram #(.AW(11), .DW(16)) u_l0 (.clk(clk), .wa(wp), .we(we), .d(wd0), .ra(ra), .q(q0));
    cloak_delayram #(.AW(11), .DW(16)) u_l1 (.clk(clk), .wa(wp), .we(we), .d(wd1), .ra(ra), .q(q1));
    cloak_delayram #(.AW(11), .DW(16)) u_l2 (.clk(clk), .wa(wp), .we(we), .d(wd2), .ra(ra), .q(q2));
        ...
        if (reset) begin
            ...
            case (st)
                ...
                S_R1: begin
                    ...
                    wd0 <= fb(x, lp0, mode == 2'd3);
                    wd1 <= fb(x, lp1, mode == 2'd3);
                    wd2 <= fb(x, lp2, mode == 2'd3);
                    st  <= S_MIX;
                end
                S_MIX: begin
                    case (mode)
...
//------------------------------------------------------------------------------
module cloak_delayram #(parameter AW = 11, parameter DW = 16) (
    input  wire           clk,
